import {
  ...
  const defaultColors = ["#FF8E9E", "#F9B8C4", "#7EC8E3", "#B0D3EA", "#C7E9B0"];
  ...
  export default function BudgetStatsCard({
    ...
    return (
      <section className="rounded-2xl border border-secondary bg-white p-5 shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
        <h3 className="text-2xl font-extrabold mb-3">{title}</h3>
  
        <div className="mb-1 text-xs text-slate-500">{totalLabel}</div>
              ...
              <PieChart>
                ...
                <Tooltip
                  formatter={(v: any, _n: any, props: any) => {
                    ...
                    return [`${formatWon(value)} (${percent}%)`, props?.payload?.name];
                  }}
                />
                <Legend verticalAlign="bottom" iconType="circle" wrapperStyle={{ paddingTop: 8 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
